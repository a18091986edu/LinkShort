...
from apps.links.managers import LinksManager
...
class LinksService:
    ...
    async def create_link(
        self, link_data: CreateLinkSchema, user: UserVerifySchema
    ) -> LinkSchema:
        """
    ...
    async def delete_link(
        self, link_data: DeleteLinkSchema, user: UserVerifySchema
    ) -> None:
        """
        ...
        if link_owner and link_owner == user.id:
            ...
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN, detail="Wrong link owner"
            )
