...
from apps.links.schemas import GetLinkSchema, LinkSchema
...
class LinksManager:
    ...
    async def get_link(self, short_link: str) -> GetLinkSchema | None:
        ...
        async with self.db.db_session() as session:
            query = select(self.link_model.full_link).where(
                self.link_model.short_link == short_link
            )
    ...
    async def get_links(self, user_id: uuid.UUID) -> list[LinkSchema]:
        ...
        async with self.db.db_session() as session:
            ...
            return [
                LinkSchema.model_validate(link, from_attributes=True) for link in links
            ]

    async def create_link(
        self, full_link: str, user_id: uuid.UUID, short_link: str
    ) -> LinkSchema:
        """
    ...
    async def get_link_owner(self, link_id: uuid.UUID) -> uuid.UUID | None:
        ...
        """
        async with self.db.db_session() as session:
            query = select(self.link_model.owner_id).where(
                self.link_model.id == link_id
            )
